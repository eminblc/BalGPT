...
| **`node` 18+ (Node.js)** | Required to run the Claude Code Bridge (`scripts/claude-code-bridge/server.js`). Needed for systemd and PM2 modes; Docker mode ships Node inside the container. Install via [nodejs.org](https://nodejs.org/) or your package manager (`apt install nodejs`, `brew install node`). |
