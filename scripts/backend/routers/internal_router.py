...
# ── Scheduler endpoint'leri — Bridge'deki Claude için ────────────────────────
...
async def internal_create_schedule(request: Request, body: _ScheduleRequest):
    ...
    Zaman daima UTC. TR saati için: TR_saat - 3 = UTC_saat.
    """
    _require_localhost(request)
    ...
    if body.cron_expr:
        ...
        if body.run_at < time.time():
            raise HTTPException(status_code=400, detail="run_at geçmişte — gelecek bir zaman olmalı")
...
async def internal_update_schedule(request: Request, task_id: str, body: _ScheduleRequest):
    ...
    # Yenisini oluştur (aynı create akışı)
    if body.cron_expr and body.run_at is not None:
        raise HTTPException(status_code=400, detail="cron_expr XOR run_at — ikisi birden kullanılamaz")
    if not body.cron_expr and body.run_at is None:
        raise HTTPException(status_code=400, detail="cron_expr veya run_at gerekli")
