...
| **`node` 18+ (Node.js)** | Claude Code Bridge'i çalıştırmak için gereklidir (`scripts/claude-code-bridge/server.js`). systemd ve PM2 modlarında host'ta zorunludur; Docker modu Node.js'i container içinde getirir. Kurulum: [nodejs.org](https://nodejs.org/) veya paket yöneticisi (`apt install nodejs`, `brew install node`). |
